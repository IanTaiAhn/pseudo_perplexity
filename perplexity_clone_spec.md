...
| 2026-07-07 | RAGAS's judge LLM wrapped via `langchain-anthropic` (Claude) and its judge embedder via `langchain-huggingface` (the project's own `sentence-transformers` model) in `ragas_runner.py` | RAGAS's OpenAI defaults | Keeps Layer 4 eval on the same "no OpenAI" path as the rest of the project instead of adding a second paid provider just for judging |
| 2026-07-07 | Pinned `langchain-community==0.3.15` (and a matching `langchain`/`langchain-core`/`langchain-openai` trio) alongside `ragas==0.2.15` | Latest `ragas` + latest `langchain-community` | `ragas` 0.2.15 unconditionally imports `langchain_community.chat_models.vertexai` at module load — a module removed in `langchain-community`'s 0.4 "sunset" release — so `import ragas` fails outright unless `langchain-community` is pinned below 0.4 |
| 2026-07-07 | Bumped `pydantic` 2.7.1 → 2.9.2 and `anthropic` 0.28.0 → 0.38.0 (not latest); `langchain-anthropic` pinned to the older 0.2.4 release | Latest `langchain-anthropic` (0.3.x, requires `anthropic>=0.41.0`); leaving `pydantic`/`anthropic` unpinned | `langchain` 0.3.30 requires `pydantic>=2.7.4`, forcing the bump. Newer `anthropic` releases (`>=0.39.0`) drop the bundled `tokenizer.json` that `chunker.py` loads at import time via `importlib.resources` — `anthropic==0.38.0` is the last release that still ships it, so `langchain-anthropic` had to be pinned to the last version (0.2.4) whose `anthropic<1` floor still permits 0.38.0. `messages.create()` itself is unchanged across this whole range, so no application code changed |
| 2026-07-08 | Layer 5: Langfuse Cloud free tier as the default; self-hosted Docker Compose stack documented but not built | Self-hosting Langfuse's own multi-service stack (Postgres + ClickHouse + Redis + MinIO) | Disproportionate infra for portfolio-project query volume; `generator.py` skips tracing entirely (no-op) if Langfuse keys are unset, so no signup is required to run the system at all |
| 2026-07-08 | Layer 5: pinned `langfuse==4.13.2` (current OTEL-based major) | `langfuse==2.x` (older `trace()`/`generation()` client) | The SDK's client API was rewritten between v2 and v3/v4; verified the real package's API in a scratch venv before writing tracing code, and confirmed `pip check` is clean against the existing `anthropic==0.38.0`/`langchain` pins |
| 2026-07-08 | Layer 5: drift detection reduces each query embedding to one scalar (cosine similarity to the baseline centroid) before running `scipy.stats.ks_2samp` | Per-dimension KS tests across all 384 embedding dimensions | A KS test is 1-D; testing every dimension independently produces ~5% false positives by chance alone and doesn't answer "has the topic mix of queries shifted" as directly as a centroid-distance summary does |
| 2026-07-08 | Layer 5: Prometheus metrics instrumented directly at the code paths that own each measurement (orchestrator, generator, query route) | A single generic FastAPI middleware measuring only HTTP-level request count/latency | The spec's metric list is domain-specific (retrieval latency, LLM tokens, cost) — a middleware outside the route has no visibility into retrieval-vs-LLM sub-stages or per-call cost |
| | | | |
